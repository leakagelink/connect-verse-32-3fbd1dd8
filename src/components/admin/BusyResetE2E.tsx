...
import { runBusyResetE2E } from "@/lib/call-invites.functions";
...
export function BusyResetE2E() {
  const run = useServerFn(runBusyResetE2E);
  ...
  const onRun = async () => {
    ...
    try {
      ...
      setResult(r);
      if (r.pass) toast.success("Busy-reset E2E: PASS");
      else toast.error(`Busy-reset E2E: FAIL — ${r.reason ?? "see steps"}`);
      console.log("[busy-reset-e2e]", r);
    } catch (e: any) {
      const msg = e?.message ?? String(e);
      setResult({ pass: false, reason: msg });
      toast.error(`E2E error: ${msg}`);
      console.error("[busy-reset-e2e] error", e);
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          <div className="text-sm font-semibold flex items-center gap-2">
            <PlayCircle className="size-4" /> Busy-Reset E2E
          </div>
          <div className="text-xs text-muted-foreground">
            Injects a stale "in_call" state for a creator, runs the reset
            pipeline, and verifies the next caller would get through.
          </div>
        </div>
        {result && (
        ...
        <div className="space-y-2 text-xs">
          ...
          {result.calleeId && (
            <div className="text-muted-foreground">
              ...
              {result.fakeCallerId && (
                <>
                  {" "}· fakeCaller: <code>{result.fakeCallerId}</code>
                </>
              )}
            </div>
          )}
